...
The deck is built on top of elmstep.pptx (ElmStepブランドのテンプレ):
- グラデ枠（深緑#123B2B→ゴールド#B99855）+ 右上「ElmStep.」ワードマーク（マスター継承）
- ペーパー背景 #F7F5F0
...
_TEMPLATE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "elmstep.pptx")
_TEMPLATE_INTRO = os.path.join(os.path.dirname(os.path.abspath(__file__)), "elmstep_intro.pptx")

# ElmStep パレット（elmstep-portfolio の :root 由来）
BRAND = {
    "green_deep": "0C231A",
    "green": "123B2B",
    "gold": "B99855",
...
class Deck:
    def __init__(self, template: str = None, include_intro: bool = True):
        """ElmStepテンプレをロードしてデッキを作る。

        include_intro=True (デフォルト): 冒頭にElmStep会社紹介スライド5枚
        (Section/Bullets ×4) を自動で含む `elmstep_intro.pptx` を使う。
        生成するスライドはこれらの後ろに追加される。
        include_intro=False: 空テンプレの `elmstep.pptx` を使う。
        """
        if template:
            path = template
            preserve = 0
