---
name: pptx_elmstep
description: ElmStepブランドのPPTX資料を作成するスキル。「PPTX作って」「パワポ作って」「資料を作って（PPTX）」「プレゼン資料」などで発動。スライドマスター＋4レイアウト（Cover/Section/Bullets/TwoCol）が組み込まれた.pptxを生成する。Marp版が欲しい場合はmarpスキルを使う。
---

# ElmStep PPTX 資料作成スキル

ElmStepブランドのスライドマスターと4つのレイアウト（Cover / Section / Bullets / TwoCol）を**.pptxファイルの中に組み込んだ状態**で出力する。納品先がPowerPointを編集するときに「新しいスライド」から4レイアウトを選べる。

## デザイン仕様

- ペーパー背景 `#F7F5F0` + 7pt のグラデ枠（左 `#123B2B` 深緑 → 右 `#B99855` ゴールド）
- 右上に「ElmStep.」ワードマーク（マスター継承／`ElmStep` は深緑、末尾の `.` はゴールド）
- 箇条書き系タイトル下に50%幅のグラデ下線（深緑→ゴールド）
- ブレットは**黒の丸（●）**、本文左端はタイトル左端と揃う
- フォント: Noto Sans JP（macOSはヒラギノfallback）
- BudouX で日本語の自然な改行
- カラーパレット（`elmstep-portfolio` の `:root` 由来。`elmstep_pptx.BRAND` にも定義）:
  | 用途 | カラー |
  |---|---|
  | 背景（ペーパー） | `#F7F5F0` |
  | テキスト（インク） | `#1C2A23` |
...
## 冒頭イントロ（ElmStep自己紹介）自動挿入

`Deck()` を引数なしで作ると、**冒頭に以下の5枚のElmStep会社紹介スライドが自動で入る**（`elmstep_intro.pptx` をベーステンプレとしてロード）。ユーザーが `d.cover()` / `d.bullets()` などで追加するスライドはこの5枚の **後ろに** 積まれる。

1. Section: 「ElmStepのご紹介」
2. Bullets: 「株式会社ElmStepについて」
3. Bullets: 「AI研修の導入実績」
4. Bullets: 「代表 溝口 喜也について」
5. Bullets: 「事業内容」
...
イントロの文言を変えたい場合は `elmstep_intro.pptx` を PowerPoint で直接編集して上書き保存する。
...
**ユーザーがNotionのURLを渡してきたら、この3ステップで生成する:**

1. Notion MCP の `notion-fetch` にURLを渡してページを取得する
2. **返ってきた出力を、加工せずそのまま** `.md` ファイルに Write する
   （`<page>` `<properties>` `<table>` 等のタグが付いたままでよい。スクリプトが処理する）
3. その `.md` を `notion_to_deck.py` に渡す

```bash
python3 notion_to_deck.py 原稿.md 出力.pptx ["表紙タイトル"] ["表紙サブタイトル"]
```

**手でMarkdownに書き起こしてはいけない。** 書き起こしミスが混入するうえ、
原稿を直すたびに同じ手作業が発生する。生出力をそのまま渡すこと。
...
末尾に `Thank you.` を自動で付ける。冒頭の会社紹介5枚は `Deck()` の既定で入る。

**表紙タイトル**は引数で明示するのが確実。省略するとNotionのページ名から推定し、
先頭の絵文字と、末尾の管理用ワード（`スライド原稿` `原稿` `台本` `ドラフト` 等＝`TITLE_SUFFIXES`）を落とす。
例:「🤖 ChatGPT実践研修（ElmStep版）スライド原稿」→ 表紙「ChatGPT実践研修（ElmStep版）」。
ページ名が管理用の名前になっている場合は、引数で渡して上書きすること。
...
cp ~/.claude/skills/pptx_elmstep/elmstep.pptx ~/dev-projects/<プロジェクト名>/
cp ~/.claude/skills/pptx_elmstep/elmstep_intro.pptx ~/dev-projects/<プロジェクト名>/
cp ~/.claude/skills/pptx_elmstep/elmstep_pptx.py ~/dev-projects/<プロジェクト名>/
```
...
d = Deck()  # elmstep_intro.pptx を自動ロード（会社紹介5枚が冒頭に入る）
...
デフォルトでは `elmstep_intro.pptx` をロードし、**冒頭にElmStep会社紹介5枚**が入った状態のデッキを作る。以降 `d.cover()` / `d.bullets()` などで追加するスライドはこの5枚の後ろに積まれる。
- `include_intro=False`: 空の `elmstep.pptx` をロード（イントロなし）
...
TwoColスライドはそもそも「サブタイトル別の対比」だから、`left_head`/`right_head` は基本的に**指定すること**を推奨。サブタイトルのフォントサイズは `elmstep_pptx.TWOCOL_HEAD_PT`（デフォルト 28pt）で調整可能。
...
### `elmstep_pptx.BRAND`
ブランドカラーのHEX辞書（`green_deep` / `green` / `gold` / `gold_hi` / `paper` / `ink` / `ink_soft`）。図形やグラフを自分で足すときに参照する。

### 文字数の自動折り返し（max_chars）

各レイアウトのプレースホルダ幅とフォントサイズに合わせて、BudouX で文節境界に折り返す。デフォルト値は `elmstep_pptx.WRAP` で調整可能:

```python
import elmstep_pptx
elmstep_pptx.WRAP["bullets_body"] = 28  # もう少し短く折る
```
...
**問題:** 日本語スライドで「プロンプトは気にしなくていい、大事なの／は／言語化力。」のように、1〜2文字だけの孤立行が生まれると極端に読みづらくなる。原因は3つあって、`elmstep_pptx.py` はそれぞれに対策を入れている:
...
- ワードマーク・グラデ枠はマスターに埋め込まれているので**スライド側に追加しない**。
- 不要なレイアウト（Title Slide, Two Content など）は既に削除済み。再追加するとPowerPointの「新しいスライド」メニューが汚れる。
- 社名表記は **株式会社ElmStep** で統一している（ポートフォリオ等の旧資料には「合同会社ElmStep」表記が残っている）。法人格が変わる場合は `elmstep_intro.pptx` を作り直すこと。
